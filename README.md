...
**v1.1.2 的目標很單純：4 分鐘內學會、玩懂，然後想再跑一局；同時加入更清楚的桌面介面與可成長的衝刺攻擊能力。**
...
直向與橫向都能玩。橫向時戰場會依可用高度放大，搖桿固定在左下、Dash / Pulse 固定在右下，更接近一般手機動作遊戲的配置；旋轉手機不需要重新開始任務。
...
- 新增「衝刺撞擊」能力線與後續等級強化。
- 手機移動由四方向按鍵改為 360° 類比虛擬搖桿，支援移動強度、斜向移動與放手自動回中。
- 新增手機橫向專用版面：戰場依螢幕高度放大，左下搖桿、右下 Dash / Pulse，可直接旋轉裝置切換。
- 保留手機 Dash / Pulse 多點觸控，並維持暫停與升級視窗不裁切。
...
├─ mobile-v112.css            # v1.1.2 類比搖桿與手機橫向版面
...
- 完整走完六步新手教學。
- 教學完成後切換到正式任務。
- 390×844 直向類比搖桿、斜向移動與放手停止。
- 844×390 橫向類比搖桿與 Dash 多點觸控。
