...
- Overdrive：移動速度 +12%。
- Reinforced Hull：最大 HULL +1，並修復 1 HULL。
- Cargo Lattice：容量 +2，Bank 分數額外 +5%。
- Phase Cooling：Dash 冷卻 -15%，最低 1.4 秒。
- Wide Pulse：Pulse 範圍 +22%。
- Capacitor：Pulse 充能速度 +20%。

每一局抽到的組合不同，因此可以走速度、容量、生存或 Pulse 等不同方向。
